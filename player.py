...
class Player(bazoklass.Baza):
    ...
    def dvishenie(self):
        super().dvishenie()
        for atacki in self.igra.protivnik.spisokatack:
            if atacki.pramoygolnik.colliderect(self.pramoygolnik_proverka) == True and self.prised != 1:
                self.igra.protivnik.spisokatack.remove(atacki)
        ...
        if klavishi[pygame.K_a] == True and self.pramoygolnik.x > - 100 and self.indikator == 0 and self.prised == 0:
            self.pramoygolnik.x = self.pramoygolnik.x - self.skorost
            self.kakoi_spisok = self.spisok_idetv_levo
            self.kakaya_storona = -1
        elif klavishi[
            pygame.K_d] == True and self.pramoygolnik.right < SCREEN_WIDTH + 150 and self.indikator == 0 and self.prised == 0:
            self.pramoygolnik.x = self.pramoygolnik.x + self.skorost
            self.kakoi_spisok = self.spisokidetvpravo
            self.kakaya_storona = 1
        elif klavishi[pygame.K_q] == True:
        ...
        elif klavishi[pygame.K_c] == True and self.indikator == 0:
            self.prised = 1
            self.kakoi_spisok = self.spisokprised if self.kakaya_storona == 1 else self.spisok_prised_vlevo
        if klavishi[pygame.K_c] == False:
            self.prised = 0

        if klavishi[pygame.K_q] == False:
